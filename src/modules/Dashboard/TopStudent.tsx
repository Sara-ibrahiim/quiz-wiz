...
<div className='border-[1px] border-[#ECECEC] rounded pe-2 m-2 flex '>
    ...
    <div className='w-11/12'>
        ...
        <p className='text-[13px] text-[#0d1321cc] dark:text-lightText '>Group: {user.group.name} {""}|{""} Average score: {Math.floor(user.avg_score)}</p>
    </div>
    <div className='pt-3'>
    <FaArrowCircleRight onClick={()=>{getStudentById(user._id)}} className=' text-2xl cursor-pointer' />
    </div>
    </div>
